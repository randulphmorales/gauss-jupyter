...
class sigma_z:
    def __init__(self, z, u_star, w_star, L, blh, fcor):
        self.z = z
        self.u_star = u_star
        self.w_star = w_star
        self.L = L
        ...
        def sigma_w(z, u_star, w_star, blh):
            if L < 0:
                sw = 0.6 * w_star
            elif L >= 0:
...
class gaussianPlume:
    ...
    def calculateConcentration(self):
        ...
        sv = self.sigma_y.sigma_v(
            self.sigma_y.z, self.sigma_y.u_star, self.blh, self.fcor)
        tsy = self.sigma_y.timescale_y(
            self.sigma_y.z, sv, self.sigma_y.u_star, self.blh, self.fcor)
        ...
        sw = self.sigma_z.sigma_w(
            self.sigma_z.z, self.sigma_z.u_star, self.sigma_z.w_star, self.blh)
        tsz = self.sigma_z.timescale_z(
            self.sigma_z.z, sw, self.sigma_z.u_star, self.blh, self.fcor)
        f_z = self.sigma_z.fz(
            self.sigma_z.z, (self.grid.xMesh - self.source.x), tsz)
        sz = self.sigma_z.sz(
